...
class DataViewer:
    def __init__(self):
        ...
        delta_r_plot = PlotboxArgs(plots=['delta_r'],
                                   labels={'left': 'delta_rudder(deg)', 'bottom': 'Time (s)'},
                                   rad2deg=True,
                                   time_window=time_window_length)
        delta_k_plot = PlotboxArgs(plots=['delta_k'],
                                   labels={'left': 'delta_kap(deg)', 'bottom': 'Time (s)'},
                                   rad2deg=True,
                                   time_window=time_window_length)
        delta_f_plot = PlotboxArgs(plots=['delta_f'],
                                   labels={'left': 'delta_flap', 'bottom': 'Time (s)'},
                                   rad2deg=False,
                                   time_window=time_window_length)
        delta_t_plot = PlotboxArgs(plots=['delta_t'],
                                   labels={'left': 'delta_thr(deg)', 'bottom': 'Time (s)'},
                                   rad2deg=False,
                                   time_window=time_window_length)
        fifth_row = [delta_r_plot, delta_k_plot, delta_f_plot, delta_t_plot]
        plots = [first_row,
                 second_row,
                 third_row,
                 fourth_row,
        ...
        self.plotter.define_input_vector('delta', ['delta_r', 'delta_k', 'delta_f', 'delta_t'])
        # plot timer
        self.time = 0.
    ...
    def update(self, true_state, estimated_state, commanded_state, delta, ts):
        ...
        delta_list = [delta.rudder, delta.kappa, delta.flap, delta.throttle]
